...
import { z } from "zod";
import { formatResponse, lineDate, lineMoney, lineNumber, linePercent, } from "./lib/formatters.js";
import { calculateMacdFromCandles, calculateRsiFromCandles, } from "./lib/indicators.js";
...
function toUserErrorMessage(tool, errors, hadNoData = false) {
...
function formatCandles(candles, limit) {
    ...
    return selected.map((candle) => `${new Date(candle.timestamp * 1000).toISOString()} | O ${candle.open.toFixed(2)} | H ${candle.high.toFixed(2)} | L ${candle.low.toFixed(2)} | C ${candle.close.toFixed(2)} | V ${Math.round(candle.volume)}`);
}
...
function formatRsiSummary(points) {
...
function formatMacdSummary(points) {
    const point = latest(points);
    if (!point) {
...
server.registerTool("get_stock_price", {
    ...
    return textResponse(formatResponse({
...
server.registerTool("get_candles", {
    ...
    const result = await executeWithFallback({
        ...
        primary: finnhubClient
            ? () => finnhubClient.getCandles(symbol, interval, from, to)
            : undefined,
        fallback: alphaVantageClient
            ? () => alphaVantageClient.getCandles(symbol, interval, from, to)
            : undefined,
    });
...
server.registerTool("get_stock_news", {
...
server.registerTool("get_rsi", {
    description: "Get RSI (Relative Strength Index). Uses provider RSI when available, otherwise computes from candles.",
    inputSchema: {
        symbol: symbolSchema,
    ...
    const providerResult = await executeWithFallback({
        toolName: "get_rsi",
        primary: finnhubClient
            ? () => finnhubClient.getRsi(symbol, interval, from, to, period)
            : undefined,
        fallback: alphaVantageClient
            ? () => alphaVantageClient.getRsi(symbol, interval, period)
            : undefined,
    });
    ...
    if (!rsiPoints.length) {
        ...
        if (!candlesResult.data?.length) {
            return textResponse(providerResult.error ?? candlesResult.error ?? "Could not compute RSI.", true);
        }
...
server.registerTool("get_macd", {
    description: "Get MACD indicator values. Uses provider MACD when available, otherwise computes from candles.",
    inputSchema: {
        symbol: symbolSchema,
        interval: intervalSchema,
        from: unixFromSchema,
    ...
    const providerResult = await executeWithFallback({
        toolName: "get_macd",
        primary: finnhubClient
            ? () => finnhubClient.getMacd(symbol, interval, from, to, fastPeriod, slowPeriod, signalPeriod)
            : undefined,
        fallback: alphaVantageClient
            ? () => alphaVantageClient.getMacd(symbol, interval)
            : undefined,
    });
    ...
    if (!macdPoints.length) {
        ...
        if (!candlesResult.data?.length) {
            return textResponse(providerResult.error ?? candlesResult.error ?? "Could not compute MACD.", true);
        }
        macdPoints = calculateMacdFromCandles(candlesResult.data, fastPeriod, slowPeriod, signalPeriod);
...
server.registerTool("get_key_financials", {
